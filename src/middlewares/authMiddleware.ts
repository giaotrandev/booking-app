  ...
  try {
    ...
    if (refreshToken) {
      try {
        ...
        if (!storedToken) {
          // Invalid refresh token, continue as guest
          req.user = undefined;
          return next();
        }
        ...
        console.log('Refresh token verification error:', refreshError);
        // Refresh token expired or invalid, continue as guest
        req.user = undefined;
        return next();
      }
    }

    // If we reach here, continue as guest user
    req.user = undefined;
    return next();
  } catch (error) {
    console.error('Optional auth middleware error:', error);
    // On server error, continue as guest to not block the request
    req.user = undefined;
    return next();
  }
};
