  ...
  try {
    ...
    await prisma.bookingHistory.create({
      data: {
        ...
        changeReason: 'VietQR Code payment method selected',
      },
    });
...
export const getPaymentQR = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const needsGeneration =
      !booking.qrCode ||
      !booking.qrCodeExpiresAt ||
      !booking.paymentWebhookReference ||
      booking.qrCodeExpiresAt <= new Date();
    ...
    if (needsGeneration) {
      ...
      // Create history record for auto-generation
      await prisma.bookingHistory.create({
        data: {
          bookingId: id,
          changedFields: {
          ...
          changedBy: userId || req.ip || 'system',
          changeReason: 'VietQR Code auto-generated on retrieval',
        },
      });

      // Update our local booking object with new data
      booking = {
        ...booking,
        qrCode: qrCodeData.qrText,
        qrCodeExpiresAt: paymentExpiration,
    ...
    let qrData;
    try {
      qrData = JSON.parse(booking.qrCode!);
    } catch (parseError) {
      console.error('Error parsing QR code data:', parseError);
      sendBadRequest(res, 'booking.qrDataCorrupted', null, language);
      return;
    }

    // Check if QR code is currently expired
    const isExpired = booking.qrCodeExpiresAt! <= new Date();

    sendSuccess(
      res,
      isExpired ? 'booking.qrExpired' : 'booking.qrRetrieved',
      {
        bookingId: booking.id,
        qrCode: isExpired ? null : qrData.url || null,
...
export const getBookingDetails = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const result = {
      ...
      pickup: {
        name: booking.pickup?.name ?? null,
        address: booking.pickup?.address ?? null,
        ward: booking.pickup?.ward.name ?? null,
        district: booking.pickup?.ward.district.name ?? null,
        province: booking.pickup?.ward.district.province.name ?? null,
      },

      dropoff: {
        name: booking.dropoff?.name ?? null,
        address: booking.dropoff?.address ?? null,
        ward: booking.dropoff?.ward.name ?? null,
        district: booking.dropoff?.ward.district.name ?? null,
        province: booking.dropoff?.ward.district.province.name ?? null,
      },

      seats: bookingTrips.flatMap((bt) => bt.seats),
...
interface ViewConfig {
  relations: string[];
  searchFields: string[];
  transformer: (booking: any) => any;
}

const BOOKING_VIEWS: Record<BookingViewMode, ViewConfig> = {
  [BookingViewMode.DEFAULT]: {
    relations: [
      'bookingTrips.trip.route.sourceProvince',
      'bookingTrips.trip.route.destinationProvince',
      'bookingTrips.trip.vehicle.vehicleType',
    ...
    searchFields: [
      'bookingCode',
      'bookingTrips.trip.route.name',
      'bookingTrips.trip.route.code',
      'pickup.name',
      'dropoff.name',
    ],
    transformer: transformBookingDefault,
  },

  [BookingViewMode.HISTORY]: {
    relations: [
      'bookingTrips.trip.route.sourceProvince',
      'bookingTrips.trip.route.destinationProvince',
      'bookingTrips.seats',
    ],
    searchFields: ['bookingCode', 'bookingTrips.trip.route.name'],
    transformer: transformBookingHistory,
  },

  [BookingViewMode.SUMMARY]: {
    relations: ['bookingTrips.trip.route.sourceProvince', 'bookingTrips.trip.route.destinationProvince'],
    searchFields: ['bookingCode'],
    transformer: transformBookingSummary,
  },

  [BookingViewMode.EXPORT]: {
    relations: [
      'bookingTrips.trip.route.sourceProvince',
      'bookingTrips.trip.route.destinationProvince',
      'bookingTrips.trip.vehicle.vehicleType',
    ...
    searchFields: ['bookingCode', 'bookingTrips.trip.route.name', 'pickup.name', 'dropoff.name'],
    transformer: transformBookingExport,
  },
};
...
/**
 * Get user bookings with view-based data optimization
 * Leverages existing queryData function for all filtering/sorting/pagination
 */
export const getUserBookings = async (req: Request, res: Response): Promise<void> => {
  const language = (req.query.lang as string) || process.env.DEFAULT_LANGUAGE || 'en';
  const userId = (req.user as { userId: string }).userId;
  ...
  try {
    // Determine view mode
    const viewMode = (req.query.view as BookingViewMode) || BookingViewMode.DEFAULT;
    const viewConfig = BOOKING_VIEWS[viewMode];

    if (!viewConfig) {
      return sendBadRequest(res, 'common.invalidQueryParams', { error: `Invalid view mode: ${viewMode}` }, language);
    }

    // Build base filters (same as original)
    const filters = buildBaseFilters(req.query, userId);

    // Build query params for queryData function
    const queryParams: DataQueryParams = {
      page: req.query.page
        ? parseInt(req.query.page as string)
        : parseInt(process.env.PAGINATION_DEFAULT_PAGE as string) || 1,
      ...
      searchFields: req.query.searchFields
        ? (req.query.searchFields as string).split(',').map((f) => f.trim())
        : viewConfig.searchFields,
      sort: parseSort(req.query),
      returnAll: req.query.returnAll === 'true',
      relations: viewConfig.relations,
      filters,
      enumFields: { status: Object.values(BookingStatus) },
    };

    // Use existing queryData function - no custom logic needed!
    const result = await queryData(prisma.booking, queryParams);

    // Transform data based on view mode
    const transformedData = await Promise.all(result.data.map(viewConfig.transformer));
...
function buildBaseFilters(query: any, userId: string) {
  ...
  if (query.busType) {
    tripFilters.vehicle = {
      vehicleType: { name: query.busType },
    };
  }

  // Trip departure time filters
  if (query.tripDepartureStart || query.tripDepartureEnd) {
    const departureDateFilter: any = {};
    if (query.tripDepartureStart) departureDateFilter.gte = new Date(query.tripDepartureStart);
    if (query.tripDepartureEnd) departureDateFilter.lte = new Date(query.tripDepartureEnd);
    tripFilters.departureTime = departureDateFilter;
  }

  // Trip arrival time filters
  if (query.tripArrivalStart || query.tripArrivalEnd) {
    const arrivalDateFilter: any = {};
    if (query.tripArrivalStart) arrivalDateFilter.gte = new Date(query.tripArrivalStart);
    if (query.tripArrivalEnd) arrivalDateFilter.lte = new Date(query.tripArrivalEnd);
    tripFilters.arrivalTime = arrivalDateFilter;
  }

  // Legacy support - Trip date range filters
  if (query.tripStartDate || query.tripEndDate) {
    const tripDateFilter: any = {};
    if (query.tripStartDate) tripDateFilter.gte = new Date(query.tripStartDate);
    if (query.tripEndDate) tripDateFilter.lte = new Date(query.tripEndDate);
    tripFilters.departureTime = tripDateFilter;
  }

  // Apply trip filters if any exist
  if (Object.keys(tripFilters).length > 0) {
    filters.bookingTrips = {
      some: { trip: tripFilters },
    };
  }
  ...
  if (query.filters) {
    try {
      const additionalFilters = JSON.parse(query.filters);
      Object.assign(filters, additionalFilters);
    } catch (e) {
      console.warn('Invalid additional filters:', query.filters);
    }
  }
...
// Định nghĩa type cho sort order
...
function parseSort(query: any): SortConfig[] {
  // Default sort
  let sort: SortConfig[] = [{ field: 'createdAt', order: 'desc' }];
  ...
  // Handle quick sort options
  const sortFilter = query.sortFilter;
...
// ===========================
// VIEW TRANSFORMERS (unchanged)
// ===========================

async function transformBookingDefault(booking: any) {
  const processedBookingTrips = await Promise.all(
    booking.bookingTrips?.map(async (bt: any) => {
      let tripImageUrl = null;
      if (bt.trip.image) {
...
async function transformBookingHistory(booking: any) {
  ...
  return {
    ...
    route: primaryTrip
      ? {
          from: primaryTrip.route.sourceProvince?.name || 'N/A',
          to: primaryTrip.route.destinationProvince?.name || 'N/A',
          departureTime: primaryTrip.departureTime,
        }
      : null,
...
async function transformBookingSummary(booking: any) {
  const primaryTrip = booking.bookingTrips?.[0]?.trip;
  ...
  return {
    ...
    finalPrice: booking.finalPrice,
    route: primaryTrip
      ? `${primaryTrip.route.sourceProvince?.name} → ${primaryTrip.route.destinationProvince?.name}`
      : null,
    departureTime: primaryTrip?.departureTime || null,
  };
}

async function transformBookingExport(booking: any) {
  const defaultData = await transformBookingDefault(booking);
